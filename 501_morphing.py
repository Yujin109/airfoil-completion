...
- 各翼型に対し CL_SHIFT を変えながら RePaint inpainting を実行
  - 高速化: 全 CL_SHIFT（× n_per_shift）を 1 回の RePaint でまとめて生成（バッチ化）
- 各生成結果について XFoil で CL/CD を計算（オプションで multiprocessing 並列化）
- 可視化を保存:
  1) 横一列（base + CL_SHIFT 別 generated）
...
def compute_clcd_batch(coords_flat: np.ndarray, angle: float, workers: int) -> Tuple[np.ndarray, np.ndarray]:
    """
    coords_flat: (M,2,L)
    workers<=1 なら逐次
    workers>1 なら multiprocessing
    """
    M = coords_flat.shape[0]
    if workers <= 1:
        cls = np.empty(M, dtype=float)
        cds = np.empty(M, dtype=float)
        for i in tqdm(range(M), desc="XFoil CL/CD (sequential)"):
            cls[i], cds[i] = get_cl_cd(coords_flat[i], angle=angle)
        return cls, cds

    import multiprocessing as mp

    ctx = mp.get_context("spawn")  # 安全側（環境差異があっても動きやすい）
    cls = np.empty(M, dtype=float)
    cds = np.empty(M, dtype=float)

    with ctx.Pool(processes=workers) as pool:
        it = pool.imap(_xfoil_worker, ((coords_flat[i], angle) for i in range(M)), chunksize=1)
        for i, (cl, cd) in enumerate(tqdm(it, total=M, desc=f"XFoil CL/CD (mp x{workers})")):
            cls[i] = cl
            cds[i] = cd
...
def plot_inpaint(orig: np.ndarray, rep: np.ndarray, mask: np.ndarray, ax, title: str = "") -> None:
    ...
    ax.plot(
        ...
        label="generated (inpainted region)",
        linestyle="-",
        color="tab:orange",
    )
...
def plot_base_with_mask(base: np.ndarray, mask: np.ndarray, ax, title: str = "") -> None:
    """
    欠損前形状（base）を表示し、mask の known/missing/blend を見えるようにする。
    """
    m = mask
    known = m == 1.0
    missing = m == 0.0
    ...
    ax.plot(base[0], base[1], color="k", linewidth=1.5, label="base (original)")
...
def add_figure_legend_unique(fig, axes, ncol: int = 5, fontsize: int = 9) -> None:
    handles, labels = [], []
    ...
    if len(uniq_h) > 0:
        fig.legend(uniq_h, uniq_l, loc="lower center", ncol=ncol, fontsize=fontsize)
...
# ---------------------------
def main():
    print("[501_morphing] ENTER main()", flush=True)
    ...
    for b_i, idx in enumerate(base_indices, start=1):
        ...
        # ---------------------------
        # Batched generation across all shifts (speed-up)
        # ---------------------------
        S = len(shifts)
        N = args.n_per_shift
        ...
        # conditioning batch
        cls_batch = cls_std.repeat(total, 1)
        cls_batch = cls_batch + (torch.from_numpy(shift_grid).to(device).view(-1, 1) / dataset.cl_std)

        # coord/mask batch
        coord_batch = coord_std.repeat(total, 1, 1)
        mask_batch = mask.repeat(total, 1)

        # seed (base 単位)
        set_seed(args.seed + b_i * 1000)

        with torch.no_grad():
            repainted_std = diffuser.repaint(model, coord_batch, mask_batch, cls_batch)

        rep_denorm_flat = dataset.denormalize_coord(repainted_std).cpu().numpy().astype(np.float32)  # (total,2,L)
        gen_all = rep_denorm_flat.reshape(S, N, 2, L)  # (S,N,2,L)
        ...
        # ---------------------------
        # XFoil CL/CD (optional parallel)
        # ---------------------------
        cl_out_flat, cd_out_flat = compute_clcd_batch(rep_denorm_flat, angle=args.angle, workers=args.xfoil_workers)
            ...
            metrics_rows.append(
                {
                    ...
                    "shift_numeric": float(shift_val),  # None も 0 扱い
        ...
        # ---------------------------
        # Plot 1) row plot (base + shift panels)
        # ---------------------------
        cols = 1 + S
        ...
        # pick representative sample (sample_id=0) for each shift panel
        rep_metrics = {}
        for r in metrics_rows:
            if r["sample_id"] == 0:
                rep_metrics[r["shift_numeric"]] = r

        for s_i, shift in enumerate(shifts):
            shift_val = float(shift_vals[s_i])
            rep = gen_all[s_i, 0]  # (2,L)

            r0 = rep_metrics.get(shift_val, None)
            if r0 is None:
                title = f"SHIFT {shift}\n(no-metrics)"
            else:
                title = (
        ...
        plt.tight_layout()
        add_figure_legend_unique(fig, axes, ncol=5, fontsize=9)
